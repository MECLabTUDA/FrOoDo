...
class MultiFileOverlappingTilesDataset(Dataset):
    def __init__(
        ...
        # Prechecks
        if crop_size != None:
            assert size[0] >= crop_size[0] and size[1] >= crop_size[1]
    ...
    def save_as_tif(self, folder, open_func=imageio.imread):
        Path(join(folder, "images")).mkdir(parents=True, exist_ok=True)
        Path(join(folder, "masks")).mkdir(parents=True, exist_ok=True)
        for image in listdir(join(self.folder, "images")):
            image_data = open_func(join(self.folder, "images", image))
            mask_data = open_func(join(self.folder, "masks", image))
            tiff.imwrite(
                join(folder, "images", image[:-3] + "tif"),
                image_data,
                photometric="rgb",
            )
            tiff.imwrite(join(folder, "masks", image[:-3] + "tif"), mask_data)
